document.addEventListener("DOMContentLoaded", () => {
  ...
  const openLocationsBtn = document.getElementById("openLocations");
  const locationModal = document.getElementById("locationModal");
  const closeModalBtn = document.getElementById("closeModal");

  /* ================= MODAL EVENTS ================= */
  if (openLocationsBtn && locationModal && closeModalBtn) {
  ...
  function updateInsight() {
    ...
    let total = 0;
    items.forEach(item => total += Number(item.dataset.download));
    ...
    if (avg >= 50) {
      ...
      tipText.textContent =
        `Your average speed at VIT, Chennai is ${avg.toFixed(
          1
        )} Mbps. Try AB1 ground floor, Gymkhana, or areas near V-Nest during off-peak hours.`;
      insightFooter.textContent =
